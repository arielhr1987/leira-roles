...
/* global ajaxurl, inlideEditRole, leiraRolesL10n */
  ...
  window.inlideEditRole = {
    ...
    init: function () {
      ...
      $('.save', row).click(function () {
        return inlideEditRole.save(this);
      });
    ...
    edit: function (id) {
      ...
      $('> div', rowData).each(function (index, value) {
        ...
        value = value.text();
        if (name === 'role') {
          $(':input[name=old_' + name + ']', editRow).val(value);
          $(':input[name=new_' + name + ']', editRow).val(value);
        } else if (name === 'capabilities') {
          //show checkboxes for capabilities
          var container = $('div.capabilities-container', editRow);
          var json = JSON.parse(value);

          //Render each capability checkbox
    ...
    save: function (id) {
      ...
      params = {
        action: 'leira-roles-quick-edit-role'
      };
      ...
      function (r) {
        ...
        if (r) {
          if (-1 !== r.indexOf('<tr')) {
            $(inlideEditRole.what + id).siblings('tr.hidden').addBack().remove();
            new_id = $(r).attr('id');
            $('#edit-' + id).before(r).remove();
            if (new_id) {
              option_value = new_id.replace(inlideEditRole.type + '-', '');
              row = $('#' + new_id);
            } else {
              option_value = id;
              row = $(inlideEditRole.what + id);
            }
    ...
    /**
     * Retrieves the ID out of the term of the element inside the table row.
     *
     * @since 2.7.0
     *
     * @memberof inlideEditRole
...
/******/ })()
;
//# sourceMappingURL=inline-edit-roles.js.map